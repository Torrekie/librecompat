// https://github.com/jiixyj/epoll-shim/blob/master/src/compat_ppoll.c
...
_ppoll_impl(struct pollfd fds[], nfds_t nfds,
    ...
    int *n)
{
	...
	int ready = poll(fds2 ? fds2 : fds, nfds + !!(fds2), timeout_ms);
	ec = ready < 0 ? errno : 0;
...
{
	int n;
	errno_t ec = _ppoll_impl(fds, nfds, timeout, sigmask, &n);
	if (ec != 0) {
		errno = ec;
		return -1;
	}
