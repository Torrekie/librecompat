...
int
compat_regexec (const regex_t *__restrict preg, const char *__restrict string,
		  ...
		  Idx stop, bool ret_len)
{
  ...
  if (length2 > 0)
	...
	if (__glibc_unlikely (s == NULL))
	  return -2;
#ifdef _LIBC
	memcpy (mempcpy (s, string1, length1), string2, length2);
#else
...
{
  return 0 == compat_regexec (&re_comp_buf, s, 0, NULL, 0);
}
...
static bool __attribute__((__noinline__))
			    ...
			    re_node_set *dests_node, bitset_t *dests_ch)
{
  ...
  __builtin_assume (ndests <= SBC_MAX);
  return ndests;
 error_return:
  for (j = 0; j < ndests; ++j)
